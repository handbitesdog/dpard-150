...
> **Descoped from this pass.** 2, 3, 4, and 6 landed: `SearchBar` filtering guide title, park name, and narrator, debounced (250ms) with an empty-results state (component-tested in `tests/components/listen.test.tsx`); `playbackService` (play/pause/seek/skip/speed, unit-tested against a fake player — see `tests/unit/playbackService.test.ts`); `MiniPlayer` wired into both the tab-bar slot and the Listen screen rows and kept in full sync through a shared `playbackStore`; and resume position written to `progressStore` on pause and on guide switch, and read back by `playbackService.play()` to seek to the saved position before playback starts. `GuideRow` did not get built as its own component — the Listen screen renders `MiniPlayer` (`variant="row"`) directly rather than wrapping it, since there was no second caller yet to justify a separate shape. The `FullPlayer` route (and with it `Scrubber`) and download UI wired to `downloadService` (`DownloadButton`) are still cut, for the same reason Phase 2 gave: each is chrome for a flow that doesn't exist yet, so building it now means guessing at its API instead of deriving it from a real caller. Skip ±15s and playback-speed are implemented and tested on `playbackService` but have no UI control invoking them — that lands with `FullPlayer`. Listen-screen grouping by park is also unbuilt; matching results still render as one flat list, just a filtered one. Resume-across-a-full-app-restart (the exit criterion below) is verified on-device: play a guide, pause partway, force-stop the process, cold-relaunch, and press play again — it seeks to the saved position before resuming, rather than restarting at 0.
